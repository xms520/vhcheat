...
typedef struct { uint32_t addr; float peak; float lastVal; int hits; int misses; char hitThis; int zone; } CandItem;
        ...
        for (int k = 0; k < g_candCount && g_appliedLast < APPLY_PER_TICK; k++) {
            ...
            if (vm_read_overwrite(task, c->addr, 4, (vm_address_t)&curV, &sz) == KERN_SUCCESS) {
                if (curV >= HP_MIN_VAL && curV <= HP_MAX_VAL) {
                    ...
                    g_appliedLast++;
                    if (appliedLogs < 10) {
                        VGLog("[hit] 0x%x %.1f -> %.1f (%s z%d)", c->addr, curV, wv,
