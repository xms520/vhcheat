...
typedef struct { uint64_t addr; double val; } SnapItem;   // v10: double/Smi 真源
            ...
            while (remain > 0 && count < MAX_SNAP_ITEMS - 4096) {
                ...
                if (b) {
                    ...
                    if (vm_read_overwrite(task, cur, chunk, (vm_address_t)b, &outSize) == KERN_SUCCESS && outSize == chunk) {
                        ...
                        size_t n32 = chunk / 4;
                        // ---- v10: 两种 V8 真源格式 ----
                        // A) HeapNumber double：8 字节 double ∈ [100,20000]，4 字节步长扫
                        //    （HeapNumber value 在对象内 offset 4 或 8，只能全对齐扫）
                        // B) Smi 64 位槽：[low32==0][high32∈[100,20000]]，槽基址记录
                        for (size_t i = 0; i < n32 && count < MAX_SNAP_ITEMS; i += 1) {
                            // C: Smi32 压缩指针（pointer compression，Cocos 3.8 默认开）
                            //    Smi = value<<1，bit0=0；394 → 内存 788
                            uint32_t sv = p32[i];
                            if ((sv & 1u) == 0 && sv >= (uint32_t)(HP_MIN_VAL * 2) && sv <= (uint32_t)(HP_MAX_VAL * 2)) {
                                buf[count].addr = (uint64_t)(cur + i * 4);
                                buf[count].val = (double)(sv >> 1);
                                count++;
                            }
                            if (i + 1 < n32) {
                                uint64_t w = ((uint64_t)p32[i+1] << 32) | p32[i];
                                double dv = *(double*)&w;
                                // A: HeapNumber double（值域 + 非 NaN）
                                if (dv >= HP_MIN_VAL && dv <= HP_MAX_VAL && dv == dv) {
                                    buf[count].addr = (uint64_t)(cur + i * 4);
                                    buf[count].val = dv;
                                    count++;
                                }
                                // B: Smi64 槽（低 32 位 0，高 32 位 = 值）
                                if (p32[i] == 0) {
                                    uint32_t smiVal = p32[i+1];
                                    if (smiVal >= (uint32_t)HP_MIN_VAL && smiVal <= (uint32_t)HP_MAX_VAL) {
                                        buf[count].addr = (uint64_t)(cur + i * 4);
                                        buf[count].val = (double)smiVal;
                                        count++;
                                    }
                                }
                            }
...
typedef struct { uint64_t addr; double peak; double lastVal; int hits; int misses; char hitThis; int zone; } CandItem;
...
static void VGDifferentialZone(int zone) {
    ...
    if (g_writtenCount > 0) {
        ...
        for (int i = 0; i < g_writtenCount && i < RECHECK_MAX; i++) {
            double v = 0; vm_size_t sz = 0;
            if (vm_read_overwrite(t, g_writtenLast[i], 8, (vm_address_t)&v, &sz) == KERN_SUCCESS) {
                if (v < 0.0001) still0++;
    ...
    if (g_godMode || g_oneHitKill) {
        ...
        for (int k = 0; k < g_candCount && g_appliedLast < APPLY_PER_TICK; k++) {
            ...
            // v11: 先读 4 字节判断格式（Smi32 vs double）
            uint32_t raw32 = 0; vm_size_t sz = 0;
            if (vm_read_overwrite(task, c->addr, 4, (vm_address_t)&raw32, &sz) == KERN_SUCCESS) {
                if ((raw32 & 1u) == 0 && raw32 >= (uint32_t)(HP_MIN_VAL*2) && raw32 <= (uint32_t)(HP_MAX_VAL*2)) {
                    // Smi32 压缩格式：写 (value<<1)
                    double curV = (double)(raw32 >> 1);
                    int32_t newSmi = g_oneHitKill ? 0 : ((int32_t)c->peak << 1);
                    uint32_t wv32 = (uint32_t)newSmi;
                    vm_write(task, c->addr, (vm_address_t)&wv32, 4);
                    g_appliedLast++;
                    if (g_writtenCount < RECHECK_MAX) g_writtenLast[g_writtenCount++] = c->addr;
                    if (appliedLogs < 3) {
                        VGLog("[hit] 0x%llx %.0f->%.0f Smi32 (%s z%d)", (unsigned long long)c->addr, curV,
                              g_oneHitKill ? 0.0 : c->peak, g_oneHitKill ? "OHK" : "GOD", zone);
                        appliedLogs++;
                    }
                } else {
                    // HeapNumber double：8 字节
                    double curV = 0;
                    if (vm_read_overwrite(task, c->addr, 8, (vm_address_t)&curV, &sz) == KERN_SUCCESS) {
                        if (curV >= HP_MIN_VAL && curV <= HP_MAX_VAL && curV == curV) {
                            double wv = g_oneHitKill ? 0.0 : c->peak;
                            vm_write(task, c->addr, (vm_address_t)&wv, 8);
                            g_appliedLast++;
                            if (g_writtenCount < RECHECK_MAX) g_writtenLast[g_writtenCount++] = c->addr;
                            if (appliedLogs < 3) {
                                VGLog("[hit] 0x%llx %.1f->%.1f dbl (%s z%d)", (unsigned long long)c->addr, curV, wv,
