...
#define TRACK_ROUNDS      6        // 滚动槽（cur + 5 轮窗口）
#define DIFF_STRIDE       5        // 差分跨 5 轮 ≈ 2 秒
#define CAND_LIMIT        4096
#define MAX_SNAP_ITEMS    2500000  // 2.5M（20MB/槽 × 6 槽 = 120MB）
#define APPLY_PER_TICK    256      // 每轮最多写入数
...
typedef struct { uint32_t addr; float peak; float lastVal; int hits; int misses; char hitThis; } CandItem;
...
static void VGDifferential(int curSlot, int prevSlot) {
    ...
    for (uint32_t i = 0; i < nCur; i++) {
        uint32_t h = (sCur[i].addr >> 2) & HB_MASK;
        hNext[i] = hHead[h];
        hHead[h] = i + 1;
    }

    // 2. 重置候选本轮命中标记
    for (int k = 0; k < g_candCount; k++) g_cands[k].hitThis = 0;

    // 3. 遍历 prev：cur 中存在且下降 [0.3%,70%] 的地址 → 更新/新增候选
                ...
                if (cv < pv) {
                    ...
                    if (ratio >= DROP_MIN_RATIO && ratio <= DROP_MAX_RATIO) {
                        ...
                        if (found >= 0) {
                            ...
                            c->hits = 1; c->misses = 0; c->hitThis = 1;
                        } else {
                            // v6: 候选满 → 淘汰一个 hits==1 的老候选换位（防僵尸占位）
                            for (int r = 0; r < g_candCount; r++) {
                                if (g_cands[r].hits <= 1 && g_cands[r].misses > 0) {
    ...
    // 4. 压缩候选表：未命中 → misses++，连续 2 次未命中才丢弃（容忍爆发间隙）
    int w = 0;
    for (int k = 0; k < g_candCount; k++) {
        if (!g_cands[k].hitThis) {
            if (++g_cands[k].misses >= 2) continue;   // 丢弃
        }
        g_cands[w++] = g_cands[k];
    }
    g_candCount = w;

    // 5. 实时应用：hits>=2 的候选 = 连续 2 个窗口都在掉 = 活体 HP → 写
    //    （地址永远来自当前快照，不存在 v4 的僵尸问题）
    g_appliedLast = 0;
        ...
        for (int k = 0; k < g_candCount && g_appliedLast < APPLY_PER_TICK; k++) {
            ...
            if (vm_read_overwrite(task, c->addr, 4, (vm_address_t)&curV, &sz) == KERN_SUCCESS) {
                if (curV >= HP_MIN_VAL && curV <= HP_MAX_VAL) {
                    float wv = g_oneHitKill ? 0.0f : c->peak;   // OHK=清零 GOD=回峰值
                    vm_write(task, c->addr, (vm_address_t)&wv, 4);
                    g_appliedLast++;
                    if (appliedLogs < 10) {
                        VGLog("[hit] 0x%x %.1f -> %.1f (%s)", c->addr, curV, wv,
                              g_oneHitKill ? "OHK" : "GOD");
                        appliedLogs++;
                    }
                }
            }
    ...
    g_tickCount++;
    if (g_tickCount % 5 == 0) {
        VGLog("[tick] snap=%zu(max=0x%x) cand=%d applied=%d mode=%s",
              nCur, nCur ? sCur[nCur-1].addr : 0, g_candCount, g_appliedLast,
